...
ARG GIT_USE_PPA_IF_AVAILABLE="false"
...
COPY library-scripts/git* /tmp/library-scripts/
RUN apt-get update \
    && apt-get install direnv -y \
    && echo 'eval "$(direnv hook zsh)"' >> /home/vscode/.zshrc \
# Use Docker script from script library to set things up
    && /bin/bash /tmp/library-scripts/docker-in-docker-debian.sh "${ENABLE_NONROOT_DOCKER}" "${USERNAME}" "${USE_MOBY}" "${DOCKER_VERSION}" \
# Install git form src
    && /bin/bash /tmp/library-scripts/git-from-src-debian.sh "${GIT_VERSION}" "${GIT_USE_PPA_IF_AVAILABLE}" \
# Install github cli
    && /bin/bash /tmp/library-scripts/github-debian.sh "${GITHUB_CLI_VERSION}"
